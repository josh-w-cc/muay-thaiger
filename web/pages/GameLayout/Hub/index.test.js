...
describe('Hub', () => {
  ...
  it('uses the shared stat-list base class for fighter details and stats', () => {
    const {container} = render(<Hub />);
    const statLists = container.querySelectorAll('dl');

    expect(statLists).toHaveLength(2);
    expect(statLists[0]).toHaveClass(statListCss.statListBase);
    expect(statLists[1]).toHaveClass(statListCss.statListBase);
  });
});
