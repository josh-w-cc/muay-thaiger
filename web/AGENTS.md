...
- **`pages/`** — Route components with React Router loaders. Each page exports a `Component` function and an optional `loader` function. Loaders fetch data from `/api/...` (relative URLs) before the route renders. Components access loader data via `useLoaderData()`.
- **`router.js`** — Route configuration using `createBrowserRouter`. Layout route modules (e.g., `GameLayout.js`) that export `Component` and `loader` are imported eagerly and referenced with `Component`/`loader` props directly. A `lazyPage(importFn, {component, loader})` helper handles screen-level modules that export named components (e.g., `FightScreen`) instead of the standard `Component` export.
