...
- **Shared CSS modules:** `components/primitive/css-modules/` contains base CSS modules (e.g., `stat-row-base.module.css`, `vertical-list-base.module.css`) that primitive React components compose from internally via CSS Modules `composes`. Page-level components should use the React primitives (e.g., `StatRow`, `VerticalList`, `GoldLabel`) rather than composing from the base CSS modules directly.
